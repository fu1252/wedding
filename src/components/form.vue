<template>
    <div class="form">
        <image class="head-img" src='/static/images/green-flower.png'></image>
        <p class="title"><span>*</span>姓名</p>
        <input type="text" placeholder="怎么称呼您" v-model="name" maxlength="6">
        <p class="title"><span>*</span>电话</p>
        <input type="number" placeholder="请填写有效的手机号码" v-model="phone" @blur='checkPhone'>
        <p class="title">几人出席</p>
        <radio-group @change='radioChange' class="group">
            <label class="radio" v-for="(item, index) in list" :key="index">
                <radio :value='item.name' :check='item.checked'></radio>
                {{item.value}}
            </label>
        </radio-group>
        <p class="title">备注</p>
        <textarea focus='true' maxlength="80" class="desc" placeholder="请填写您的备注请求"
        name='textarea' placeholder-style="color:#ccc" v-model="desc"></textarea>
        <div class="btn">
            <button class="left" @tap='submit'>确定提交</button>
